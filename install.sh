...
function Preference {
	...
	read -p"$GREEN[#MR.kia#]$WHITE-->" Theme
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT THEME\n(1)LIGHT\n(2)DARK\n(3)HIGH-CONTRAST\n\n"
		read -p"$GREEN[#MR.kia#]$WHITE-->" Language
...
function Mail_Options {
	...
	if [ $Opt == 1 ];
		...
		read -p"$GREEN[#MR.kia#]$WHITE-->" recipient
		while [ "$recipient" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
			read -p"$GREEN[#MR.kia#]$WHITE-->" recipient
		...
		read -sp"$GREEN[#MR.kia#]$WHITE-->" password
		while [ "$password" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
			read -sp"$GREEN[#MR.kia#]$WHITE-->" password
		...
		read -p"$GREEN[#MR.kia#]$WHITE-->" destination
		while [ "$destination" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
			read -p"$GREEN[#MR.kia#]$WHITE-->" destination
		...
		read -p"$GREEN[#MR.kia#]$WHITE-->" server
		while [ "$server" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
			read -p"$GREEN[#MR.kia#]$WHITE-->" server
		...
		read -p"$GREEN[#MR.kia#]$WHITE-->" port
		while [ "$port" = "" ];
			do
...
function Options {
	...
	read -sp"$GREEN[#MR.kia#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#MR.kia#]$WHITE-->" up_pass
	...
	read -p"$GREEN[#MR.kia#]$WHITE-->" Token
	while [ "$Token" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ACCESS YOUR DATABASE ON OTHER DEVICES(ON THE SAME NETWORK)?(1)YES(2)NO\n\n"
		read -p"$GREEN[#MR.kia#]$WHITE-->" Token
	...
	read -p"$GREEN[#MR.kia#]$WHITE-->" Access
	while [ "$Access" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ADD SOME CREDENTIALS FOR ACCESS THE DATABASE?(1)YES(2)NO\n\n"
		read -p"$GREEN[#MR.kia#]$WHITE-->" Access
	...
	cd Configuration
	echo ";THIS FILE HAS BEEN GENERATE BY MR.kia INSTALLER">Configuration.ini
...
echo '{
	...
	echo ";THIS FILE HAS BEEN GENERATE BY MR.kia INSTALLER">Configuration/Configuration.ini
	...
	echo "password = kia">>Configuration/Configuration.ini
	...
	printf "\n${WHITE}UPDATE-PASSWORD:${GREEN}kia\n"
...
function installer {
	...
	if [ $confvar == 1 ];
		...
		read -p "$GREEN[#MR.kia#]$WHITE-->" selected
		while [ "$selected" == "" ];
			do
			printf "${BLUE}\n\nWOULD YOU LIKE TO SET(1)MANUAL-INSTALLATION(2)AUTO-INSTALLATION\n\n"
			read -p "$GREEN[#MR.kia#]$WHITE-->" selected
		...
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED Mr.kia\n\n"
		exit 0
	fi
